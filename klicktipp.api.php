...
class KlicktippConnector {
   ...
   */
  public function __construct($service = 'http://api.klickmail.com.br') {
   ...
   *
   * @return An object representing the Klicktipp user object.
   */
  public function user_index() {
    $response = $this->_http_request('/account');

    if (empty($response->error)) {
      return !isset($response->data) ? NULL : $response->data;
    } else {
      $this->error = 'User index failed: '.$response->error;
      return FALSE;
    }
  }

  /**
   * Get a user from the website. Requires to be logged in.
   *
  ...
  public function user_get($uid) {
    ...
    // retrieve
    $response = $this->_http_request('/account/'.urlencode($uid));

    if (empty($response->error)) {
      return !isset($response->data) ? NULL : $response->data;
    } else {
      $this->error = 'User get failed: '.$response->error;
      return FALSE;
    }
  }

  /**
   * Get a user by login name. Requires to be logged in.
   *
   * @param mixed $username The user login name.
   *
   * @return An object representing the Klicktipp user object.
   */
  public function user_search($username) {
    ...
    $response = $this->_http_request('/account/search', 'POST', $data);

    if (empty($response->error)) {
      return !isset($response->data) ? NULL : $response->data;
    } else {
      $this->error = 'User search failed: '.$response->error;
      return FALSE;
    }
  }
   ...
   * @param array $fields (optional) Additional user fields.
   *
   * @return object The user object of the created user.
   */
  public function user_create($username, $password, $email, $firstname, $lastname, $fields = array()) {
    ...
    $response = $this->_http_request('/account', 'POST', $data);

    if (empty($response->error)) {
      return !isset($response->data) ? NULL : $response->data;
    } else {
      $this->error = 'User creation failed: '.$response->error;
      return FALSE;
    }
  }

  /**
   * Updates the account of a user. Requires to be logged in.
   *
   * @param mixed $uid The user id used to identifie which user to modifie.
   * @param array $fields The user fields to update.
   */
  public function user_update($uid, $fields = array()) {
    ...
    // update user
    $data = $fields;
    $response = $this->_http_request('/account/'.urlencode($uid), 'PUT', $data);

    if (empty($response->error)) {
      return !isset($response->data) ? NULL : $response->data;
    } else {
      $this->error = 'User update failed: '.$response->error;
      return FALSE;
    }
  }
   ...
   *
   * @param mixed $uid The user id of the user to delete.
   */
  public function user_delete($uid) {
    ...
    // delete user
    $response = $this->_http_request('/account/'.urlencode($uid), 'DELETE');

    if (empty($response->error)) {
      return TRUE;
    } else {
      $this->error = 'User deletion failed: '.$response->error;
      return FALSE;
    }
  }

  /**
   * Get the user groups. Requires to be logged in.
   *
   * @return An list of the Klicktipp user groups.
   */
  public function group_index() {
    $response = $this->_http_request('/group');

    if (empty($response->error)) {
      return !isset($response->data) ? NULL : $response->data;
    } else {
      $this->error = 'Group index failed: '.$response->error;
      return FALSE;
    }
  }

  /**
   * Get group definition. Requires to be logged in.
   *
   * @param mixed $gid The id of the group
   *
   * @return An object representing the Klicktipp user group.
   */
  public function group_get($gid) {
    if (!$gid) {
      $this->error = 'Illegal Arguments';
      return FALSE;
    }
    ...
    // retrieve
    $response = $this->_http_request('/group/'.urlencode($gid));

    if (empty($response->error)) {
      return !isset($response->data) ? NULL : $response->data;
    } else {
      $this->error = 'Group get failed: '.$response->error;
      return FALSE;
    }
  }
